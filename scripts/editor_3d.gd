## editor_3d.gd  –  Version 2.0
## Viewport 3D + logique des outils.
## Nouveautés : Undo/Redo, peinture par drag, pipette, copier/coller,
##              snap de hauteur, flèches caméra, contours jaunes sur sélection.
extends Node
...
signal texture_picked(fc: MD.FaceConfig)   ## Pipette : texture récupérée
signal map_changed                          ## Pour minimap et recalculs externes
...
## "tx,ty,si" → {mi: MeshInstance3D, body: StaticBody3D}
var _cube_nodes : Dictionary = {}

# ── Orbite caméra ────────────────────────────────────────────────────────────
var _cam_distance  : float = 12.0
var _cam_elevation : float = 45.0
## 225° = vue classique iso SW→NE (corrige le décalage apparent)
...
func _build_viewport() -> void:
	...
	sun.rotation_degrees = Vector3(-50, 30, 0); sun.light_energy = 1.3
	_sub_viewport.add_child(sun)

	var fill := DirectionalLight3D.new()
	fill.rotation_degrees = Vector3(-20, -150, 0)
	fill.light_energy = 0.4; fill.shadow_enabled = false
...
func _rebuild_tile(tx: int, ty: int) -> void:
	for si in [-1, 0, 1, 2, 3]:
		...
		if _cube_nodes.has(key):
			_cube_nodes[key]["mi"].queue_free(); _cube_nodes.erase(key)
	var td := map_data.get_tile(tx, ty)
	if td == null: return
	if not td.subdivided:
		_spawn_cube(tx, ty, -1, td.cubes[0], MD.TILE_SIZE)
	else:
		for si in 4: _spawn_cube(tx, ty, si, td.cubes[si], MD.TILE_SIZE * 0.5)
...
func _spawn_cube(tx: int, ty: int, si: int, cube: MD.CubeData, cube_size: float) -> void:
	...
	cshape.shape = shape; body.add_child(cshape); mi.add_child(body)
...
static func _cube_world_pos(tx: int, ty: int, si: int, cube_size: float) -> Vector3:
	var TS := MD.TILE_SIZE
	var bx := float(tx) * TS; var bz := float(ty) * TS
	if si < 0 or cube_size >= TS: return Vector3(bx, 0.0, bz)
...
func rebuild_cube(tx: int, ty: int, si: int) -> void:
	...
	if _cube_nodes.has(key):
		...
		var cshape := CollisionShape3D.new(); var shape := BoxShape3D.new()
		var top_h := cube.top_max(); var by := cube.base_y
		var height := maxf(top_h - by, 0.1)
		shape.size = Vector3(cube_size * 0.98, height, cube_size * 0.98)
		cshape.position = Vector3(0, by + height * 0.5, 0)
		cshape.shape = shape; body.add_child(cshape)
	else:
...
func _center_camera_on_map() -> void:
	## Centre géométrique réel de l'emprise de la grille (milieu des carreaux + bordure)
	var TS := MD.TILE_SIZE
...
## Déplace le rig caméra dans une direction axiale (touches fléchées)
...
func reset_camera() -> void: _center_camera_on_map()

# ══════════════════════════════════════════════════════════════════════════════
# INPUT (forwarded depuis SubViewportContainer)
# ══════════════════════════════════════════════════════════════════════════════
...
func _on_mouse_motion(event: InputEventMouseMotion) -> void:
	if _is_orbiting:
		...
		if _is_left_held: _do_tool_drag(event.position)

# ══════════════════════════════════════════════════════════════════════════════
# RAYCAST
# ══════════════════════════════════════════════════════════════════════════════
func _raycast(mouse_pos: Vector2) -> Dictionary:
	if not _physics_ready or _camera == null: return {}
...
func _hit_to_cube_info(hit: Dictionary) -> Dictionary:
	...
	return {
		"tx": mi.get_meta("tx"), "ty": mi.get_meta("ty"), "si": mi.get_meta("si"),
		"pos": hit.get("position", Vector3.ZERO), "normal": hit.get("normal", Vector3.UP),
	}

func _hit_face_idx(normal: Vector3) -> int:
	var ax : float = absf(normal.x); var ay : float = absf(normal.y); var az : float = absf(normal.z)
	if ay >= ax and ay >= az: return MD.FACE_TOP if normal.y > 0 else MD.FACE_BOTTOM
	if ax >= az: return MD.FACE_EAST if normal.x > 0 else MD.FACE_WEST
...
func _nearest_grid_corner_xz(hit_pos: Vector3) -> Vector2:
	var TS := MD.TILE_SIZE
	return Vector2(roundf(hit_pos.x / (TS * 0.5)) * (TS * 0.5),
...
func _do_tool_hover(mouse_pos: Vector2) -> void:
	...
	var tx := info["tx"] as int; var ty := info["ty"] as int; var si := info["si"] as int
...
func _build_tile_outline_mesh(tx: int, ty: int, si: int,
	...
	var hx  := cs * 0.5 + 0.035; var hz := hx
	var yt  := cube.top_max() + 0.035; var yb := cube.base_y - 0.01
	...
	var mesh := ArrayMesh.new(); mesh.add_surface_from_arrays(Mesh.PRIMITIVE_LINES, arr)
	var mat  := StandardMaterial3D.new()
	mat.albedo_color = color
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	mat.cull_mode    = BaseMaterial3D.CULL_DISABLED
	var mi := MeshInstance3D.new(); mi.position = pos; mi.mesh = mesh
	mi.set_surface_override_material(0, mat)
	return mi
...
func _show_tile_outline(tx: int, ty: int, si: int, color: Color) -> void:
	...
	if mi: _sel_root.add_child(mi); _hover_tile_outline = mi
...
func _show_corner_spheres(corners: Array, color: Color) -> void:
	for cr in corners:
		...
		(mi.mesh as SphereMesh).radius = 0.10; (mi.mesh as SphereMesh).height = 0.20
		mi.set_surface_override_material(0, mat)
		_sel_root.add_child(mi); _hover_corner_spheres.append(mi)
...
func _show_face_highlight(tx: int, ty: int, si: int, fi: int, mat: Material) -> void:
	...
	var cs  := MD.TILE_SIZE if (not td.subdivided) else MD.TILE_SIZE * 0.5
	var mi  := MeshInstance3D.new()
	mi.position = _cube_world_pos(tx, ty, si, cs); mi.mesh = MB.build_cube(cube, cs * 1.01)
	var invis := StandardMaterial3D.new()
	invis.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA; invis.albedo_color = Color(0,0,0,0)
	for s in 6: mi.set_surface_override_material(s, invis)
	mi.set_surface_override_material(fi, mat)
	_sel_root.add_child(mi); _hover_face_plane = mi
...
func _do_tool_click(mouse_pos: Vector2) -> void:
	if not _physics_ready:
		status_message.emit("Initialisation en cours, veuillez patienter…"); return
	var hit  := _raycast(mouse_pos)
	var info := _hit_to_cube_info(hit)
	if info.is_empty(): clear_selection(); return

	var tx := info["tx"] as int; var ty := info["ty"] as int; var si := info["si"] as int
	var hit_pos := info["pos"] as Vector3; var normal := info["normal"] as Vector3
	...
	match current_tool:
		...
		Tool.EYEDROPPER:
			...
			if cube and cube.face_configs[fi].has_texture():
				...
				status_message.emit("💧 Cette face n'a pas de texture — utilisez l'outil 🖌 pour en appliquer")

# ══════════════════════════════════════════════════════════════════════════════
# OUTIL — DRAG (bouton gauche maintenu + déplacement)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# AJUSTEMENT DE HAUTEUR (avec Undo/Redo + Snap)
# ══════════════════════════════════════════════════════════════════════════════
...
## Rappelé par undo_redo.undo() / redo()
func _restore_cube_snapshot(state: Dictionary, sel: Array, is_faces: bool) -> void:
	...
	for tk in tiles:
		var p : PackedStringArray = tk.split(","); _rebuild_tile(int(p[0]), int(p[1]))
...
func copy_selected() -> void:
	...
	status_message.emit("✓ Tuile copiée (hauteurs + textures)")

func paste_selected() -> void:
	if _clipboard_cube == null:
		status_message.emit("Presse-papiers vide — copiez d'abord une tuile"); return
	var targets := {}
	for sf in selected_faces:
...
# ══════════════════════════════════════════════════════════════════════════════
# OVERLAY DE SÉLECTION — contours JAUNES pour toute tuile sélectionnée
# ══════════════════════════════════════════════════════════════════════════════
...
func _refresh_selection_overlay() -> void:
	...
	var outlined := {}   ## clés déjà contourées

	## ── Coins sélectionnés ──────────────────────────────────────────────────
	for cr in selected_corners:
		## Contour jaune de la tuile
		var ok := "%d,%d,%d" % [cr["tx"],cr["ty"],cr["si"]]
		if not outlined.has(ok):
			outlined[ok] = true
			var cube := map_data.get_cube(cr["tx"], cr["ty"], cr["si"])
			if cube:
				var mi := _build_tile_outline_mesh(cr["tx"],cr["ty"],cr["si"],cube,
					Color(1.0, 0.85, 0.1, 1.0))
				if mi: _sel_root.add_child(mi)
		## Sphère bleue sur le coin
		var p    := map_data.get_corner_world_xz(cr["tx"],cr["ty"],cr["si"],cr["ci"])
		var cube := map_data.get_cube(cr["tx"],cr["ty"],cr["si"])
		var y    := (cube.corners[cr["ci"]] + 0.06) if cube else 0.5
		...
		(smi.mesh as SphereMesh).radius = 0.12; (smi.mesh as SphereMesh).height = 0.24
		smi.set_surface_override_material(0, smat); _sel_root.add_child(smi)
	...
	for sf in selected_faces:
		...
		if not outlined.has(ok):
			outlined[ok] = true
			var omi := _build_tile_outline_mesh(sf["tx"],sf["ty"],sf["si"],cube,
				Color(1.0, 0.85, 0.1, 1.0))
			if omi: _sel_root.add_child(omi)
		## Surbrillance bleue de la face
		var hi_mesh := MB.build_cube(cube, cs * 1.015)
		var fmi     := MeshInstance3D.new()
		fmi.position = _cube_world_pos(sf["tx"],sf["ty"],sf["si"],cs); fmi.mesh = hi_mesh
		var invis    := StandardMaterial3D.new()
		invis.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA; invis.albedo_color = Color(0,0,0,0)
...
func toggle_test_mode(enabled: bool) -> void:
	_cube_root.visible = not enabled; _grid_root.visible = not enabled
...
func export_to_json_map() -> Dictionary:
	...
	for ty in map_data.grid_height:
		var row : Array = []
		for tx in map_data.grid_width:
			var td := map_data.get_tile(tx, ty); var cube := td.cubes[0]
...
func set_tool(t: int) -> void:
	current_tool = t; clear_selection()
